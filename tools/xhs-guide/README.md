...
这几条不靠模型自觉：**只要工具真的查到了条目，条目区就由服务端按真实字段确定性
拼装**——`collectMaterial` 从工具结果里取字段，按优先级轮流给每条加一段，加到接近
上限为止。模型只负责判断帖子类型、调对工具、挑中哪几条。这样做的原因是两次实测
教训：让模型"改写到 260 字"时它会**编出不存在的房源**来凑长度；只在越界时才接管
时，模型自己写的行里混着 `rent unknown`、`月租面议`、`｜yes` 这种脏列直译。
服务端还会删掉聊天页话术（"已放宽关键词""如仍不满意…我再为您调整"，legacy
searchWanted 写在 action 里的）、删掉重复条目、删掉结尾散文。
