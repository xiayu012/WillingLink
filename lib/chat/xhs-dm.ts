 ...
 *
 * `*` 必须**不跟着第二个星号**：房源标题是 `**标题** ([原帖](…))`，开头那两个
 * 星号是加粗，不是列表符号。旧版把加粗标题也算成字段行，于是 `isListingTitle`
 * 对**所有加粗标题永远返回 false**——整段被当成一个块，`capToLimit` 一"丢最后
 * 一条房源"就把全部房源丢光，私信里只剩结尾那句总结。标题不加粗时看不出来，
 * 所以这个 bug 藏了很久（2026-08-26 才发现）。
 */
const FIELD_LINE = /^[ \t]*(?:[-•]|\*(?!\*))/;
 ...
 *
 * 模型输出形状固定：一行标题（不带 `-`）+ 若干 `- ` 开头的信息点，循环，前面常有
 * 一句引导语、后面常有一句总结。只看"下一行是不是 `-` 开头"就够分辨。
 *
 * **这个骨架是 prompts.ts 那边跟这里的约定**，别单方面改。2026-08-26 把房源
 * 展示从固定字段表改成"AI 自己挑重点"时，第一版把首条写成了裸文本段落——标题行
 * 的下一行不再是 `-` 开头，分割线会整个失效（而且不报错，只是悄悄不插）。
 * 变的应该是每条写什么、按什么顺序写，不是这个骨架。
 *
 * 判据本身：
 *
