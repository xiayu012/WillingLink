...
| 标记 | 含义 | 要不要管 |
|---|---|---|
| ✗ CODE_BUG | 库里**存在**满足硬约束的房源，但没返回/返回了违反约束的 | **要修** |
| ⚠ 提示缺失 | 结果实际放宽了（跨城等）但没给用户 relaxedNote | 建议修 |
| ⚠ 判 0 分 | 硬约束都过，但 LLM 认为语义不相关 | 人工看一眼 |
| ◌ DATA_GAP | 库里**不存在**满足的房源（含湾区外需求），搜索已尽力 | 不用管，等数据 |

判定原理：每条查询先在全量房源上算 ground truth（可证明满足预算/城市±邻市/
房型的房源集合），ground truth 为空时的"结果不理想"归为数据问题而非代码问题。
...
确定性：评测强制 `SEARCH_DETERMINISTIC=1`（`pickBest` 固定取 rank-1），
同一代码 + 同一数据下结果可复现；线上仍保持加权随机以支持"换一个"。
